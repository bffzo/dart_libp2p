...
import 'package:dart_libp2p/core/network/rcmgr.dart'
    show ConnScope; // Import ConnScope
...
class SecuredConnection implements TransportConn {
  ...
  SecuredConnection(
    ...
    // ADDED LOGGING
    _log.finer(
      'SecuredConnection Constructor: Peer ${establishedRemotePeer?.toString() ?? 'unknown'}',
    );
  ...
  Future<Uint8List> read([int? length]) async {
    _log.finer(
      'SecuredConnection.read: Called with length: $length, _decryptedBuffer.length: ${_decryptedBuffer.length}',
    ); // ADDED
    if (length == 0) {
      _log.finer(
        'SecuredConnection.read: Requested length is 0, returning empty Uint8List.',
      ); // ADDED
    ...
    if (length == null) {
      if (!_decryptedBuffer.isEmpty) {
        ...
        _log.finer(
          'SecuredConnection.read (length=null): Returning buffered data of length ${data.length}. Data preview: ${data.take(20).toList()}',
        ); // ADDED
        return data;
      }
      final decryptedMessage = await _readAndDecryptMessage();
      _log.finer(
        'SecuredConnection.read (length=null): Returning directly from _readAndDecryptMessage, length ${decryptedMessage.length}. Data preview: ${decryptedMessage.take(20).toList()}',
      ); // ADDED
    ...
    while (_decryptedBuffer.length < length) {
      ...
      try {
        _log.finer(
          'SecuredConnection.read (length=$length): _decryptedBuffer.length (${_decryptedBuffer.length}) < requested ($length). Calling _readAndDecryptMessage().',
        ); // ADDED
        decryptedChunk = await _readAndDecryptMessage();
      } catch (e) {
        _log.finer(
          'SecuredConnection.read (length=$length): Error in _readAndDecryptMessage: $e. Current _decryptedBuffer.length: ${_decryptedBuffer.length}',
        ); // ADDED
      ...
      if (decryptedChunk.isEmpty) {
        ...
        _log.finer(
          'SecuredConnection.read (length=$length): _readAndDecryptMessage returned empty (EOF). Breaking loop.',
        ); // ADDED
        break;
      }
      ...
      _log.finer(
        'SecuredConnection.read (length=$length): Added ${decryptedChunk.length} bytes to _decryptedBuffer. New _decryptedBuffer.length: ${_decryptedBuffer.length}',
      ); // ADDED
    }

    if (_decryptedBuffer.isEmpty) {
      _log.finer(
        'SecuredConnection.read (length=$length): _decryptedBuffer is empty after loop. Returning empty Uint8List.',
      ); // ADDED
      return Uint8List(0);
    }
    ...
    if (_decryptedBuffer.length >= length) {
      ...
      _log.finer(
        'SecuredConnection.read (length=$length): Returning ${dataToReturn.length} bytes. Remaining _decryptedBuffer.length: ${_decryptedBuffer.length}. Data preview: ${dataToReturn.take(20).toList()}',
      ); // ADDED
      return dataToReturn;
    } else {
      ...
      _log.finer(
        'SecuredConnection.read (length=$length): Returning ${dataToReturn.length} bytes (less than requested due to EOF). _decryptedBuffer is now empty. Data preview: ${dataToReturn.take(20).toList()}',
      ); // ADDED
      return dataToReturn;
    }
  }
  ...
  Future<Uint8List> _readFullMessage(
    int expectedLength, {
    Duration? timeout,
  }) async {
    timeout ??=
        const Duration(seconds: 30); // Default timeout for message reads
    ...
    while (buffer.length < expectedLength) {
      ...
      _log.finer(
        'SecuredConnection: _readFullMessage attempt $readAttempts: received ${chunk.length} bytes',
      );
  ...
  Future<Uint8List> _readAndDecryptMessage() async {
    ...
    _log.finer(
      'SecuredConnection: Attempting decryption with nonce: ${nonce.toList()}',
    );
    ...
    try {
      ...
      _log.finer(
        'SecuredConnection: Decryption successful, got ${plaintext.length} bytes',
      );
  ...
  Future<void> write(Uint8List data) async {
    ...
    // ADDED LOGGING for hashCode
    _log.finer(
      'SecuredConnection: Using encryption key (hashCode: ${_encryptionKey.hashCode}): ${await _encryptionKey.extractBytes()}',
    );
    ...
    _log.finer(
      'SecuredConnection: Encrypted data length: ${secretBox.cipherText.length}',
    );
    ...
    _log.finer(
      'SecuredConnection:   Raw Ciphertext to send: ${hex.encode(secretBox.cipherText)}',
    );
    _log.finer(
      'SecuredConnection:   Raw MAC to send: ${hex.encode(secretBox.mac.bytes)}',
    );
    ...
    _log.finer(
      'SecuredConnection: Writing ${data.length} bytes as $dataLength bytes encrypted+MAC',
    );
    _log.finer(
      'SecuredConnection:   Raw Ciphertext to send: ${hex.encode(secretBox.cipherText)}',
    );
    ...
    _log.finer(
      'SecuredConnection: First 4 bytes of encrypted: ${secretBox.cipherText.take(4).toList()}',
    );
    _log.finer(
      'SecuredConnection: TO_UNDERLYING_WRITE - Length: ${combinedData.length}, Bytes: ${hex.encode(combinedData)}',
    );
