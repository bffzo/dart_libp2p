...
import 'package:dart_libp2p/core/crypto/keys.dart' as keys;
import 'package:dart_libp2p/core/crypto/pb/crypto.pbenum.dart'
    as crypto_pb; // Renamed
...
class NoiseSecurity implements SecurityProtocol {
  ...
  static const String _protocolString = protocolIdForState;
  ...
  Future<Uint8List> _readEncryptedPayload(SecuredConnection conn) async {
    final lengthBytes = await conn.read(
      2,
    ); // Assuming SecuredConnection.read handles underlying raw reads
    if (lengthBytes.length < 2) {
      throw NoiseProtocolException('Failed to read payload length');
    }
  ...
  Future<void> _writeEncryptedPayload(
  ...
  Future<Uint8List> _readLibp2pHandshakePayload(SecuredConnection conn) async {
    ...
    final decryptedOuterFrame = await conn
        .read(); // Read one full message decrypted by SecuredConnection
    if (decryptedOuterFrame.length < 2) {
      throw NoiseProtocolException('Payload too short after decryption');
    }
    final actualPayloadLength =
        (decryptedOuterFrame[0] << 8) | decryptedOuterFrame[1];
    if (decryptedOuterFrame.length != 2 + actualPayloadLength) {
      throw NoiseProtocolException(
        'Decrypted payload length mismatch: expected ${2 + actualPayloadLength}, got ${decryptedOuterFrame.length}',
      );
    }
  ...
  Future<SecuredConnection> secureOutbound(TransportConn connection) async {
    ...
    try {
      ...
      await connection
          .write(await pattern.writeMessage(Uint8List(0))); // -> s, se
      ...
      // ADDED LOGGING
      _log.finer(
        'NoiseSecurity.secureOutbound: Handshake complete. Pattern keys:',
      );
      _log.finer(
        '  - pattern.sendKey.hashCode: ${pattern.sendKey.hashCode}, pattern.sendKey.bytes: ${await pattern.sendKey.extractBytes()}',
      );
      _log.finer(
        '  - pattern.recvKey.hashCode: ${pattern.recvKey.hashCode}, pattern.recvKey.bytes: ${await pattern.recvKey.extractBytes()}',
      );
      ...
      final tempSecuredConn = SecuredConnection(
        ...
        pattern.recvKey,
        securityProtocolId:
            _protocolString, // Temporary, won't have peer details yet
      );
      ...
      final signature =
          await _identityKey.privateKey.sign(pattern.remoteStaticKey);
      initiatorPayload.identitySig = signature;

      await _writeEncryptedPayload(
        tempSecuredConn,
        initiatorPayload.writeToBuffer(),
      );

      // 2. Receive and process responder's payload
      final responderEncryptedPayloadBytes =
          await _readLibp2pHandshakePayload(tempSecuredConn);
      final responderPayload = noise_pb.NoiseHandshakePayload.fromBuffer(
        responderEncryptedPayloadBytes,
      );
      ...
      // ADDED LOGGING
      _log.finer(
        'NoiseSecurity.secureOutbound: Libp2p handshake payload processed. Finalizing SecuredConnection with pattern keys:',
      );
      _log.finer(
        '  - pattern.sendKey.hashCode: ${pattern.sendKey.hashCode}, pattern.sendKey.bytes: ${await pattern.sendKey.extractBytes()}',
      );
  ...
  Future<SecuredConnection> secureInbound(TransportConn connection) async {
    ...
    try {
      ...
      final initiatorEncryptedPayloadBytes =
          await _readLibp2pHandshakePayload(tempSecuredConn);
      final initiatorPayload = noise_pb.NoiseHandshakePayload.fromBuffer(
        initiatorEncryptedPayloadBytes,
      );
      ...
      await _writeEncryptedPayload(
        tempSecuredConn,
        responderPayload.writeToBuffer(),
      );
      ...
      // ADDED LOGGING
      _log.finer(
        'NoiseSecurity.secureInbound: Libp2p handshake payload processed. Finalizing SecuredConnection with pattern keys:',
      );
      _log.finer(
        '  - pattern.sendKey.hashCode: ${pattern.sendKey.hashCode}, pattern.sendKey.bytes: ${await pattern.sendKey.extractBytes()}',
      );
