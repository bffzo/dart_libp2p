...
const Duration RecentlyConnectedAddrTTL = AddressTTL.recentlyConnectedAddrTTL;
const Duration ConnectedAddrTTL = AddressTTL.connectedAddrTTL;
const Duration TempAddrTTL = AddressTTL.tempAddrTTL;
...
const int legacyIDSize = 2 * 1024;
const int signedIDSize = 8 * 1024;
const int maxOwnIdentifyMsgSize = 4 *
      ...
      if (currentRecord is Envelope && otherRecord is Envelope) {
        ...
        try {
          ...
          if (currentRecordBytes.length != otherRecordBytes.length) {
            ...
            for (var i = 0; i < currentRecordBytes.length; i++) {
...
class IdentifyService implements IDService {
  ...
  void _setupObservedAddrManager() {
    ...
    _observedAddrMgr = ObservedAddrManager(
      ...
      interfaceListenAddrs: () async => host.network.interfaceListenAddresses,
    );
  ...
  Future<bool> _updateSnapshot() async {
    _log.fine(
      'IdentifyService._updateSnapshot: Called. Querying host.addrs. Host: ${host.id}, Host hashCode: ${host.hashCode}',
    );
    final addrs = host.addrs;
    _log.finer(
      'IdentifyService._updateSnapshot: host.addrs returned: $addrs for host ${host.id}',
    );
    ...
    return _currentSnapshotMutex.synchronized(() async {
  ...
  Future<P2PStream?> _newStreamAndNegotiate(
    ...
    _log.warning(
      ' [IDENTIFY-STREAM-CREATE-START] Creating stream for peer=$peerId, protocol=$protoIDString, conn_age=${connAge.inMilliseconds}ms',
    );
    ...
    try {
      ...
      _log.warning(
        ' [IDENTIFY-STREAM-CREATE-ATTEMPT] About to call conn.newStream() for peer=$peerId, protocol=$protoIDString',
      );

      stream = await conn.newStream(
        Context(),
      ); // Context and StreamID are placeholders if not used by underlying muxer for initial open
      final streamCreateDuration = DateTime.now().difference(streamCreateStart);

      // DEBUG: Log successful stream creation
      _log.warning(
        ' [IDENTIFY-STREAM-CREATE-SUCCESS] Stream created for peer=$peerId, stream_id=${stream.id()}, protocol=$protoIDString, duration=${streamCreateDuration.inMilliseconds}ms',
      );
      ...
      _log.warning(
        ' [IDENTIFY-PROTOCOL-NEGOTIATE-START] Starting protocol negotiation for peer=$peerId, stream_id=${stream.id()}, protocol=$protoIDString',
      );
      final negotiateStart = DateTime.now();

      final protocolMuxer = host.mux as MultistreamMuxer;
      final selectedProtocol =
          await protocolMuxer.selectOneOf(stream, [protoIDString]);
      final negotiateDuration = DateTime.now().difference(negotiateStart);

      // DEBUG: Log protocol negotiation result
      _log.warning(
        ' [IDENTIFY-PROTOCOL-NEGOTIATE-RESULT] Protocol negotiation for peer=$peerId, stream_id=${stream.id()}, requested=$protoIDString, selected=$selectedProtocol, duration=${negotiateDuration.inMilliseconds}ms',
      );

      if (selectedProtocol == null) {
        _log.warning(
          '🔧 [NEWSTREAM-STEP-3-FAILED] Protocol negotiation failed for peer=$peerId, stream_id=${stream.id()}, protocol=$protoIDString. Resetting stream.',
        );
        await stream.reset();
        return null;
      }
      ...
      stream.setDeadline(DateTime.now().add(timeout));
      _log.warning(
        ' [IDENTIFY-DEADLINE-RESET] Reset stream deadline after negotiation for peer=$peerId, stream_id=${stream.id()}, new_deadline=${DateTime.now().add(timeout).toIso8601String()}',
      );

      // DEBUG: Log protocol assignment
      _log.warning(
        ' [IDENTIFY-PROTOCOL-ASSIGN-START] Setting protocol scope and stream protocol for peer=$peerId, stream_id=${stream.id()}, protocol=$selectedProtocol',
      );
      ...
      _log.warning(
        ' [IDENTIFY-PROTOCOL-ASSIGN-STREAM] Setting stream protocol for peer=$peerId, stream_id=${stream.id()}, protocol=$selectedProtocol',
      );
      ...
      _log.warning(
        ' [IDENTIFY-STREAM-CREATE-COMPLETE] Stream creation and negotiation complete for peer=$peerId, stream_id=${stream.id()}, protocol=$selectedProtocol',
      );
  ...
  Future<Identify> _createBaseIdentifyResponse(
    ...
    mes.protocols.addAll(snapshot.protocols.map((p) => p));
    mes.observedAddr = remoteAddr.toBytes();
    _log.finer(
      'IdentifyService._createBaseIdentifyResponse: ObservedAddr set to $remoteAddr for ${conn.remotePeer}',
    );
  ...
  Future<void> handleIdentifyResponse(P2PStream stream, bool isPush) async {
    final peer = stream.conn.remotePeer;
    final side = stream.conn.localPeer == host.id ? 'CLIENT' : 'SERVER';
    ...
    try {
      ...
      if (metricsTracer != null) {
        metricsTracer!.identifyReceived(
          isPush,
          mes.protocols.length,
          mes.listenAddrs.length,
        );
      }
  ...
  Future<Identify> _readAllIDMessages(P2PStream stream) async {
    ...
    for (var i = 0; i < maxMessages; i++) {
      ...
      try {
        ...
        _log.severe(
          ' [READ-ALL-ID-MESSAGES-CHUNK-${i + 1}-ERROR] Error reading chunk from peer=$peer, duration=${chunkErrorTime.inMilliseconds}ms, error=$e',
        );

        // Check for specific stream closed / EOF conditions
        if (e is StateError &&
            (e.message.contains('remote side closed (EOF)') ||
                e.message.contains('YamuxStreamState.closed') ||
                e.message.contains('closed by remote') ||
                e.message.contains('Stream is YamuxStreamState.reset'))) {
          break;
        }
        if (e is StreamClosedException ||
            (e is IOException && e.toString().contains('closed'))) {
          break;
        }
        _log.severe(
          ' [READ-ALL-ID-MESSAGES-CHUNK-${i + 1}-UNHANDLED] Unhandled error reading message chunk from peer=$peer: $e. Rethrowing.',
        );
        rethrow;
      }
    }
  ...
  Future<void> _consumeMessage(Identify mes, Conn conn, bool isPush) async {
    ...
    var ttl = RecentlyConnectedAddrTTL;
    ...
    await _consumeReceivedPubKey(
      conn,
      Uint8List.fromList(mes.publicKey),
    ); // Await this

    if (obsAddr != null) {
      _evtPeerIdentificationCompleted.emit(
  ...
  Future<Envelope?> signedPeerRecordFromMessage(Identify msg) async {
    ...
    try {
      final (envelope, _) = await Envelope.consumeEnvelope(
        Uint8List.fromList(msg.signedPeerRecord),
  ...
  Future<List<MultiAddr>> _consumeSignedPeerRecord(
    ...
    PeerId id;
    try {
      id = PeerId.fromPublicKey(
        signedPeerRecord.publicKey,
      ); // Added null check operator
      _log.finer(
  ...
  Future<void> _consumeReceivedPubKey(Conn conn, Uint8List? kb) async {
    ...
    if (np != rp) {
      ...
      if (rp.toString().isEmpty && np.toString().isNotEmpty) {
        ...
        try {
          host.peerStore.keyBook.addPubKey(
            rp,
            newKey,
          ); // Use original rp if it's a placeholder that needs updating
          _log.fine(
  ...
  List<MultiAddr> _filterAddrs(List<MultiAddr> addrs, MultiAddr remote) {
    _log.finer(
      'IdentifyService._filterAddrs: Filtering ${addrs.length} addresses based on remote address type: $remote (Loopback: ${remote.isLoopback()}, Private: ${remote.isPrivate()}, Public: ${remote.isPublic()})',
    );
  ...
  (List<ProtocolID>, List<ProtocolID>) _diff(
    List<ProtocolID> before,
    List<ProtocolID> after,
  ) {
    _log.finer(
    ...
    final beforeSet = Set<ProtocolID>.from(before);
    final afterSet = Set<ProtocolID>.from(after);
    ...
    await _connsMutex.synchronized(() async {
      ...
      if (entry != null) {
        ...
        entry = _addConnWithLock(
          conn,
        ); // _addConnWithLock returns the new/existing entry
        entry.identifyWaitCompleter = Completer<void>();
        completerToAwait = entry.identifyWaitCompleter;
        _spawnIdentifyConn(conn, entry);
      }
  ...
  void _spawnIdentifyConn(Conn conn, Entry entry) {
    ...
    _identifyConn(conn).then((_) {
      ...
      _evtPeerIdentificationFailed.emit(
  ...
  Future<void> _identifyConn(Conn conn) async {
    ...
    try {
      ...
      if (stream == null) {
        ...
        throw Exception(
          'Failed to open or negotiate identify stream with $peerId',
        );
      }
