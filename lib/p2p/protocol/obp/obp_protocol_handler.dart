...
class OBPProtocolHandler {
  ...
  static Future<bool> performHandshake(
    ...
    try {
      if (isClient) {
        return await _performClientHandshake(
          stream,
          timeout: timeout,
          context: context,
        );
      } else {
        return await _performServerHandshake(
          stream,
          timeout: timeout,
          context: context,
        );
  ...
  static Future<OBPFrame?> readFrame(
    ...
    return _readFrame(stream, timeout: timeout, context: context);
  }
  ...
  static Future<OBPFrame?> _readFrame(
    ...
    try {
      ...
      final headerData = await _readExactBytes(
        stream,
        OBPFrame.HEADER_SIZE,
        timeout: timeout,
        context: context,
      );
      if (headerData == null) {
      ...
      final payloadLength =
          headerBuffer.getUint32(8); // Length field at offset 8

      // Read payload if present
      Uint8List? payloadData;
      if (payloadLength > 0) {
        payloadData = await _readExactBytes(
          stream,
          payloadLength,
          timeout: timeout,
          context: context,
