  ...
  static HttpRequest parse(Uint8List data, PeerId remotePeer) {
    ...
    if (headerEndIndex == -1) {
      throw const FormatException(
        'Invalid HTTP request: no header separator found',
      );
    }
...
class HttpRoute {
  ...
  Map<String, String> extractParams(String path) {
    ...
    final paramNames = RegExp(':([a-zA-Z_][a-zA-Z0-9_]*)')
...
class HttpProtocolService {
  ...
  Future<void> _handleRequest(P2PStream stream, PeerId peerId) async {
    ...
    _logger.info(
      '🎯 [HTTP-SERVER-START] HTTP request handler started for peer $peerId. Stream ID: ${stream.id}',
    );
    ...
    try {
      ...
      if (requestData.isEmpty) {
        _logger.warning(
          '⚠️ [HTTP-SERVER-PHASE-2] Received empty request from peer $peerId',
        );
        return;
      }
      ...
      final totalTime = DateTime.now().difference(startTime);
      _logger.severe(
        '❌ [HTTP-SERVER-ERROR] Error handling HTTP request from peer $peerId after ${totalTime.inMilliseconds}ms: $e\n$stackTrace',
      );
  ...
  Future<HttpResponse> request(
    ...
    final startTime = DateTime.now();
    _logger.info(
      '🚀 [HTTP-REQUEST-START] Making HTTP request to peer $peerId: ${method.value} $path',
    );

    // Phase 1: Stream Creation
    _logger.info(
      '📡 [HTTP-REQUEST-PHASE-1] Creating new stream to peer $peerId',
    );
