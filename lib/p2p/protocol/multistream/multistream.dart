...
class MultistreamMuxer implements ProtocolSwitch {
  ...
  Future<List<ProtocolID>> protocols() async {
    return _handlerLock.synchronized(() async {
  ...
  Future<ProtocolID?> selectOneOf(
    ...
    try {
      ...
      for (var i = 0; i < protocolsToSelect.length; i++) {
  ...
  Future<Uint8List> _performReadDelimited(P2PStream stream) async {
    ...
    throw TimeoutException(
      'Read operation failed after $maxRetries retries',
      readTimeout,
    );
  }
  ...
  Future<Uint8List> _performSingleReadDelimited(
    ...
    return _actualReadDelimited(stream).timeout(
      readTimeout,
      onTimeout: () => throw TimeoutException(
        'Multistream read operation timed out',
        readTimeout,
      ),
    );
  }
  ...
  Future<Uint8List> _actualReadDelimited(P2PStream stream) async {
    ...
    var length = -1;
    var bytesRead = 0;
    var varintBytesRead = 0;
    var lengthDecoded = false;
      ...
      if (!lengthDecoded) {
        ...
        try {
          final (decodedLength, consumed) =
              MultiAddrCodec.decodeVarint(buffer.toBytes());
          if (consumed > 0) {
            length = decodedLength;
            lengthDecoded = true;
            varintBytesRead = consumed;
            if (length > 1024) {
              await YamuxExceptionUtils.safeStreamReset(
                stream,
