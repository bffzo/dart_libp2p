...
extension ReservationExtension on CircuitV2Client {
  ...
  Future<Reservation> _reserveStream(
    ...
    print(
      '[Client] _reserveStream started for relay: ${relayPeerId.toBase58()}',
    );
    try {
      ...
      print(
        '[Client] Response received, type: ${responseMsg.type}, status: ${responseMsg.status}',
      );
      ...
      if (responseMsg.status != pb.Status.OK) {
        print(
          '[Client] ERROR: Reservation failed with status: ${responseMsg.status.name}',
        );
        throw ReservationError(
      ...
      final expireInSeconds = rsvpData.expire.toInt();
      final expiration =
