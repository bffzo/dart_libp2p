...
import 'package:dart_libp2p/core/network/network.dart'
    show Network, Reachability; // Conn is in its own file
...
import 'package:dart_libp2p/p2p/host/autonat/dial_policy.dart';
import 'package:dart_libp2p/p2p/host/autonat/options.dart';
import 'package:dart_libp2p/p2p/host/autonat/service.dart';
...
class AmbientAutoNAT implements AutoNAT, Notifiee {
  ...
  void _handleEventBusEvent(dynamic event) {
    ...
    if (event is EvtLocalAddressesUpdated) {
      ...
      _log(
        'Handling EvtPeerIdentificationCompleted for peer ${event.peer}.',
      );
      ...
      _host.peerStore.protoBook
        ...
        if (supportedProtocols.contains(autoNATV1Proto)) {
          _log(
            'Peer ${event.peer} supports AutoNAT. Forcing probe schedule update.',
          );
          _onAddressOrPeerChange(forceProbeOverride: true);
        } else {
          _log('Peer ${event.peer} does not support AutoNAT.');
        }
      }).catchError((e) {
        if (_isClosed) return;
        _log('Error checking protocols for peer ${event.peer}: $e');
      });
    } else {
  ...
  void _scheduleNextProbe(bool forceProbe) {
    ...
    var nextProbeAfter = _config.refreshInterval;
    ...
    var nextProbeTime = _lastProbe.add(nextProbeAfter);
...
class StaticAutoNAT implements AutoNAT {
  // Can have an associated service
