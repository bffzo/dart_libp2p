import 'dart:async';
import 'dart:io'
    show InternetAddressType, NetworkInterface; // Added for NetworkInterface
...
import 'package:dart_libp2p/p2p/host/basic/internal/backoff/backoff.dart';
...
class BasicHost implements Host {
  ...
  BasicHost._({
    ...
    _log.fine(
      '[BasicHost CONSTRUCTOR] for host $id - Initial _network.listenAddresses: ${_network.listenAddresses}',
    );
  }
  ...
  Future<void> start() async {
    _log.fine(
      '[BasicHost start] BEGIN. Host ID: $id, network.hashCode: ${_network.hashCode}, initial network.listenAddresses: ${_network.listenAddresses}',
    );
    _log.fine(
  ...
  Future<void> _startBackground() async {
    ...
    return _monitorAddressChanges();
  }

  /// Emits an address change event when the host's addresses change.
  Future<void> _emitAddressChangeEvent(
    List<MultiAddr> prev,
  ...
  Future<void> _monitorAddressChanges() async {
    ...
    _addressMonitorTimer = Timer.periodic(const Duration(seconds: 5), (_) {
      // Store the timer
      if (_closed) return;
        ...
        for (final interface in interfaces) {
          ...
          for (final address in interface.addresses) {
            ...
            try {
              ...
              _log.finer('Added interface address: $ma');
            } catch (e) {
              _log.severe(
  ...
  Future<void> _newStreamHandler(P2PStream stream) async {
  ...
  void signalAddressChange() {
    _log.fine(
      '[BasicHost signalAddressChange] Called. _addrChangeChan.isClosed: ${_addrChangeChan.isClosed}, _closed: $_closed. Host: $id',
    );
  ...
  List<MultiAddr> get allAddrs {
    ...
    for (final listenAddr in currentListenAddrs) {
      ...
      if (isUnspecified) {
        ...
        var ipComponentIndex = -1;

        // Find the index of the first IP component
        for (var i = 0; i < listenComponents.length; i++) {
          ...
          if (_filteredInterfaceAddrs.isNotEmpty) {
            for (final interfaceAddr in _filteredInterfaceAddrs) {
              ...
              try {
                ...
                _log.finer(
                  'Resolved unspecified listen addr: $listenAddr with interface $interfaceAddr to $newAddr',
                );
              } catch (e) {
                _log.severe(
    ...
    _log.fine(
      '[BasicHost allAddrs] for host $id - resolvedAddrs: $resolvedAddrs',
    );
    _log.fine(
      '[BasicHost allAddrs] for host $id - natAppliedAddrs: $natAppliedAddrs',
    );
    _log.fine(
      '[BasicHost allAddrs] for host $id - finalAddrs (before toList): $finalAddrs',
    );
    _log.fine(
      '[BasicHost allAddrs] for host $id - Returning: $result',
    );
    return result;
  }
  ...
  List<MultiAddr> get publicAddrs {
    ...
    _log.fine(
      '[BasicHost publicAddrs] for host $id - Observed: ${observed.length}, Final result: ${result.length} addresses: $result',
    );
    return result;
  }
  ...
  Future<void> connect(AddrInfo pi, {Context? context}) async {
    ...
    await peerStore.addrBook
        .addAddrs(pi.id, filteredAddrs, const Duration(minutes: 5));
  ...
  Future<void> _dialPeer(PeerId p, Context context) async {
    ...
    try {
      ...
      final totalTime = DateTime.now().difference(startTime);
      _log.severe(
        '❌ [DIAL-PEER-ERROR] Failed to dial $p after ${totalTime.inMilliseconds}ms: $e\n$stackTrace',
      );
