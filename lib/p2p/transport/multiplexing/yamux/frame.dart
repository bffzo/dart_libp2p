...
class YamuxFrame {
  ...
  factory YamuxFrame.fromBytes(Uint8List bytes) {
    ...
    // Parse stream ID
    final streamId = header.getUint32(4);
  ...
  Uint8List toBytes() {
    ...
    // Write flags
    buffer.setUint16(2, flags);

    // Write stream ID
    buffer.setUint32(4, streamId);

    // Write length
    buffer.setUint32(8, length);
  ...
  static YamuxFrame windowUpdate(int streamId, int delta) {
    final data = ByteData(4)..setUint32(0, delta);
    return YamuxFrame(
  ...
  static YamuxFrame goAway(int reason) {
    final data = ByteData(4)..setUint32(0, reason);
    return YamuxFrame(
