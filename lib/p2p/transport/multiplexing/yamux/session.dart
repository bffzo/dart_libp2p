...
import 'package:dart_libp2p/core/network/context.dart'; // Explicit import for Context
import 'package:dart_libp2p/core/network/mux.dart'
    as core_mux; // Explicit import for MuxedConn, MuxedStream
import 'package:dart_libp2p/core/network/rcmgr.dart'; // Explicit import for PeerScope
import 'package:dart_libp2p/core/network/stream.dart';
import 'package:dart_libp2p/core/network/transport_conn.dart';
import 'package:dart_libp2p/core/peer/peer_id.dart';
import 'package:dart_libp2p/p2p/transport/multiplexing/multiplexer.dart'; // This now brings in core_mux.MuxedConn and PeerScope via its own imports
...
class YamuxSession implements Multiplexer, core_mux.MuxedConn, Conn {
  ...
  void _init() {
    _readFrames().catchError((error, stackTrace) {
      if (!_closed) {
        ...
        _log.fine(
          '$_logPrefix Error in _readFrames after session closed: $error',
          stackTrace,
        );
      }
    });
  ...
  Future<void> _sendPing() async {
    ...
    try {
      ...
      _log.warning(
        '$_logPrefix Error sending PING: $e. Session may be unhealthy.',
      );
  ...
  Future<void> _readFrames() async {
    ...
    final recentFrameProcessingTimes = <Duration>[];

    // High-volume mode detection to reduce logging overhead
    var isHighVolumeMode = false;
    var recentFrameCount = 0;
    var lastFrameCountReset = DateTime.now();

    void updateVolumeMode() {
      final now = DateTime.now();
      if (now.difference(lastFrameCountReset) > const Duration(seconds: 1)) {
        isHighVolumeMode =
            recentFrameCount > 50; // 50+ frames/sec = high volume
        recentFrameCount = 0;
        lastFrameCountReset = now;

        if (isHighVolumeMode) {
          _log.fine(
            '$_logPrefix Entering high-volume mode (50+ frames/sec) - reducing logging overhead',
          );
        }
      }
    }
    ...
    try {
      while (!_closed) {
        ...
        logFrameProcessing(
          '$_logPrefix 🔧 [YAMUX-FRAME-READER-LOOP-$frameCount] Starting frame read iteration. Buffer size: ${buffer.length}, Session closed: $_closed, Connection closed: ${_connection.isClosed}',
        );
        ...
        if (framesInCurrentBatch >= _maxFramesPerBatch) {
          ...
          logFrameProcessing(
            '$_logPrefix 🔧 [YAMUX-FRAME-READER-BATCH-YIELD] Applied adaptive delay after $frameCount frames (batch size: $_maxFramesPerBatch)',
          );
        } else {
        ...
        logFrameProcessing(
          '$_logPrefix 🔧 [YAMUX-FRAME-READER-HEADER-$frameCount] Need $headerSize bytes for header, have ${buffer.length} bytes',
        );
        while (buffer.length < headerSize) {
          if (_closed || _connection.isClosed) {
            // Check before read
            _log.warning(
              '$_logPrefix 🔧 [YAMUX-FRAME-READER-EXIT-$frameCount] Loop condition met (_closed=$_closed, _conn.isClosed=${_connection.isClosed}) before reading for header. Exiting.',
            );
          ...
          _log.fine(
            '$_logPrefix 🔧 [YAMUX-FRAME-READER-HEADER-READ-$frameCount] About to read $bytesNeeded bytes for header from connection',
          );
          ...
          _log.fine(
            '$_logPrefix 🔧 [YAMUX-FRAME-READER-HEADER-READ-RESULT-$frameCount] Read ${chunk.length} bytes in ${readDuration.inMilliseconds}ms (requested $bytesNeeded)',
          );

          if (chunk.isEmpty) {
            // Connection closed by peer
            _log.warning(
              '$_logPrefix 🔧 [YAMUX-FRAME-READER-EOF-$frameCount] Connection closed by peer while reading header. Cleaning up.',
            );
            await _cleanupWithoutFrames();
            return;
          }
          ...
          _log.fine(
            '$_logPrefix 🔧 [YAMUX-FRAME-READER-HEADER-PROGRESS-$frameCount] Header buffer now has ${buffer.length}/$headerSize bytes',
          );
        }

        _log.fine(
          '$_logPrefix 🔧 [YAMUX-FRAME-READER-HEADER-COMPLETE-$frameCount] Header read complete, parsing frame info',
        );

        final headerView =
            ByteData.view(buffer.buffer, buffer.offsetInBytes, headerSize);
        final bodyLength = headerView.getUint32(8);
        final expectedTotalFrameLength = headerSize + bodyLength;

        _log.fine(
          '$_logPrefix 🔧 [YAMUX-FRAME-READER-BODY-$frameCount] Frame body length: $bodyLength, total expected: $expectedTotalFrameLength, current buffer: ${buffer.length}',
        );
        ...
        while (buffer.length < expectedTotalFrameLength) {
          if (_closed || _connection.isClosed) {
            // Check before read
            _log.warning(
              '$_logPrefix 🔧 [YAMUX-FRAME-READER-EXIT-BODY-$frameCount] Loop condition met (_closed=$_closed, _conn.isClosed=${_connection.isClosed}) before reading for body. Exiting.',
            );
            await _cleanupWithoutFrames();
            return;
          }
          ...
          _log.fine(
            '$_logPrefix 🔧 [YAMUX-FRAME-READER-BODY-READ-$frameCount] About to read $stillNeeded bytes for frame body from connection',
          );
          ...
          _log.fine(
            '$_logPrefix 🔧 [YAMUX-FRAME-READER-BODY-READ-RESULT-$frameCount] Read ${chunk.length} bytes in ${bodyReadDuration.inMilliseconds}ms (requested $stillNeeded)',
          );

          if (chunk.isEmpty) {
            // Connection closed by peer
            _log.warning(
              '$_logPrefix 🔧 [YAMUX-FRAME-READER-EOF-BODY-$frameCount] Connection closed by peer while reading body (header indicated bodyLength $bodyLength). Cleaning up.',
            );
            await _cleanupWithoutFrames();
            return;
          }
          ...
          _log.fine(
            '$_logPrefix 🔧 [YAMUX-FRAME-READER-BODY-PROGRESS-$frameCount] Body buffer now has ${buffer.length}/$expectedTotalFrameLength bytes',
          );
        }

        _log.fine(
          '$_logPrefix 🔧 [YAMUX-FRAME-READER-PARSE-$frameCount] Full frame received, parsing frame from $expectedTotalFrameLength bytes',
        );
        ...
        _log.fine(
          '$_logPrefix 🔧 [YAMUX-FRAME-READER-RAW-$frameCount] Raw frame bytes (${frameBytesForParser.length}): ${frameBytesForParser.take(64)}...',
        );

        if (frame.length != bodyLength) {
          _log.severe(
            '$_logPrefix 🔧 [YAMUX-FRAME-READER-ERROR-$frameCount] Frame body length mismatch! Header said $bodyLength, frame parser said ${frame.length}. Frame: ${frame.type}, StreamID: ${frame.streamId}, Flags: ${frame.flags}',
          );
          await _goAway(YamuxCloseReason.protocolError);
          return;
        }
        if (frame.data.length != bodyLength) {
          // Should be redundant if YamuxFrame.fromBytes is correct
          _log.severe(
            '$_logPrefix 🔧 [YAMUX-FRAME-READER-ERROR-$frameCount] Frame data actual length mismatch! Header said bodyLength $bodyLength, frame.data.length is ${frame.data.length}. Frame: ${frame.type}, StreamID: ${frame.streamId}, Flags: ${frame.flags}',
          );
          await _goAway(YamuxCloseReason.protocolError);
          return;
        }
        ...
        logFrameProcessing(
          '$_logPrefix 🔧 [YAMUX-FRAME-READER-LOOP-COMPLETE-$frameCount] Frame $frameCount processed in ${loopDuration.inMilliseconds}ms, buffer remaining: ${buffer.length} bytes',
        );
      }
    } catch (e, st) {
      final connIsClosed = _connection.isClosed; // Capture current state
      if (!_closed && !connIsClosed) {
        ...
        _log.fine(
          '$_logPrefix _readFrames: Underlying connection found closed, error during read: $e',
          st,
        );
        await _cleanupWithoutFrames();
      } else {
        // _closed is true
        _log.fine(
          '$_logPrefix _readFrames: Error after session already marked closed: $e',
          st,
        );
      }
    } finally {
      if (!_closed) {
        _log.warning(
          '$_logPrefix _readFrames loop exited unexpectedly while session not marked closed. Forcing cleanup.',
        );
        await _goAway(
          YamuxCloseReason.internalError,
        ); // This will also call _cleanupWithoutFrames
      }
      _log.fine(
        '$_logPrefix _readFrames loop exited. Final state: _closed=$_closed, ConnClosed=${_connection.isClosed}',
      );
    }
  }
  ...
  Future<void> _handleFrame(YamuxFrame frame) async {
    final handleStartTime = DateTime.now();
    _log.fine(
      '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME] START: Type=${frame.type}, StreamID=${frame.streamId}, Flags=${frame.flags}, Length=${frame.length}, Session closed: $_closed',
    );
    ...
    if (_closed &&
        ...
        frame.type != YamuxFrameType.reset) {
      _log.warning(
        '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-SKIP] Session closed, ignoring frame type ${frame.type} for stream ${frame.streamId}',
      );
      return;
    }
    ...
    try {
      ...
      switch (frame.type) {
        ...
        default:
          _log.severe(
            '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-UNKNOWN] Unknown frame type: ${frame.type} for stream ${frame.streamId}',
          );
      }

      final handleDuration = DateTime.now().difference(handleStartTime);
      _log.fine(
        '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-SUCCESS] Frame handled successfully in ${handleDuration.inMilliseconds}ms: Type=${frame.type}, StreamID=${frame.streamId}',
      );
    } catch (e, st) {
      final handleDuration = DateTime.now().difference(handleStartTime);
      _log.severe(
        '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-ERROR] Error handling frame after ${handleDuration.inMilliseconds}ms: Type=${frame.type}, StreamID=${frame.streamId}, Error: $e\n$st',
      );
      rethrow;
    }
  }
  ...
  Future<void> _handleDataFrame(YamuxFrame frame) async {
    _log.fine(
      '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-DATA] Processing DATA frame for stream ${frame.streamId}, length: ${frame.length}, flags: ${frame.flags}',
    );
    ...
    if (streamData != null) {
      _log.fine(
        '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-DATA-DISPATCH] Dispatching DATA frame to stream ${frame.streamId}, stream state: ${streamData.streamState}',
      );
      ...
      _log.fine(
        '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-DATA-COMPLETE] Stream ${frame.streamId} handled DATA frame in ${streamHandleDuration.inMilliseconds}ms',
      );
    } else {
      _log.warning(
  ...
  Future<void> _handleWindowUpdateFrame(YamuxFrame frame) async {
    _log.fine(
      '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-WINDOW] Processing WINDOW_UPDATE frame for stream ${frame.streamId}',
    );
    final stream = _streams[frame.streamId];
    if (stream != null) {
      _log.fine(
        '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-WINDOW-DISPATCH] Dispatching WINDOW_UPDATE to stream ${frame.streamId}',
      );
      await stream.handleFrame(frame);
      _log.fine(
        '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-WINDOW-COMPLETE] Stream ${frame.streamId} handled WINDOW_UPDATE',
      );
    } else {
      _log.warning(
        '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-WINDOW-ERROR] Received ${frame.type} for unknown/closed stream ID ${frame.streamId}. Flags: ${frame.flags}, Length: ${frame.length}',
      );
    }
  }
  ...
  Future<void> _handleNewStreamFrame(YamuxFrame frame) async {
    _log.fine(
      '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-NEWSTREAM] Processing NEW_STREAM frame for stream ${frame.streamId}, flags: ${frame.flags}',
    );

    if (frame.flags & YamuxFlags.syn != 0 &&
        frame.flags & YamuxFlags.ack != 0) {
      _log.fine(
        '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-SYN-ACK] Received SYN-ACK for outgoing stream ID ${frame.streamId}',
      );
      final completer = _pendingStreams.remove(frame.streamId);
      if (completer != null && !completer.isCompleted) {
        completer.complete();
        _log.fine(
          '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-SYN-ACK-COMPLETE] Completed pending stream completer for ID ${frame.streamId}',
        );
      } else {
        _log.warning(
          '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-SYN-ACK-ERROR] Received SYN-ACK for stream ID ${frame.streamId}, but no pending completer found or already completed.',
        );
      }
    } else if (frame.flags & YamuxFlags.syn != 0) {
      _log.fine(
        '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-SYN] Processing incoming SYN for stream ${frame.streamId}',
      );
      await _handleNewStream(frame);
    } else {
      _log.warning(
        '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-NEWSTREAM-ERROR] Received NEW_STREAM frame with unexpected flags: ${frame.flags} for stream ID ${frame.streamId}',
      );
  ...
  Future<void> _handleResetFrame(YamuxFrame frame) async {
    _log.fine(
      '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-RESET] Processing RESET frame for stream ${frame.streamId}',
    );
    final stream = _streams[frame.streamId];
    if (stream != null) {
      _log.fine(
        '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-RESET-DISPATCH] Dispatching RESET to stream ${frame.streamId}',
      );
      await stream.handleFrame(frame);
      _log.fine(
        '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-RESET-COMPLETE] Stream ${frame.streamId} handled RESET',
      );
    } else {
      _log.fine(
        '$_logPrefix 🔧 [YAMUX-HANDLE-FRAME-RESET-UNKNOWN] Received RESET for unknown/closed stream ID ${frame.streamId} (this is normal for cleanup)',
      );
    }
  }
  ...
  Future<void> _handleNewStream(YamuxFrame frame) async {
    ...
    _log.fine(
      '$_logPrefix _handleNewStream: START for incoming stream ID ${frame.streamId}. Flags: ${frame.flags}',
    ); // Elevated log level
    if (frame.flags & YamuxFlags.syn == 0) {
    ...
    _log.fine(
      '$_logPrefix _handleNewStream: Attempting to send SYN-ACK for stream ID ${frame.streamId}.',
    ); // Elevated log level
    try {
      await _sendFrame(ackFrame);
      _log.fine(
        '$_logPrefix _handleNewStream: Successfully sent SYN-ACK for stream ID ${frame.streamId}.',
      ); // Elevated log level
    } catch (e) {
      _log.severe(
        '$_logPrefix _handleNewStream: FAILED to send SYN-ACK for stream ID ${frame.streamId}: $e. Aborting stream setup.',
    ...
    _log.fine(
      '$_logPrefix _handleNewStream: Creating local YamuxStream for ID ${frame.streamId}. Initial local receive window: $initialWindow',
    ); // Elevated log level
    ...
    try {
      await stream.open();
      _log.fine(
        '$_logPrefix _handleNewStream: Local YamuxStream ID ${frame.streamId} opened.',
      ); // Elevated log level
      _log.fine(
        '$_logPrefix _handleNewStream: Adding fully initialized stream ID ${frame.streamId} to _incomingStreamsController.',
      ); // Elevated log level
      _incomingStreamsController.add(stream);

      if (_streamHandler != null) {
        _log.fine(
          '$_logPrefix _handleNewStream: Invoking _streamHandler for stream ID ${frame.streamId}.',
        ); // Elevated log level
  ...
  Future<void> _handlePing(YamuxFrame frame) async {
    ...
    _log.finer(
      '$_logPrefix Received PING request, opaque: $opaqueValue. Sending PONG.',
    );
  ...
  Future<void> _handleGoAway(YamuxFrame frame) async {
    _log.fine(
      '$_logPrefix Received GO_AWAY frame. Reason code: ${frame.length}. Closing session.',
    );
  ...
  Future<void> _sendFrame(YamuxFrame frame) async {
    if (_closed && !_cleanupStarted) {
      ...
      if (frame.type != YamuxFrameType.goAway) {
        _log.warning(
          '$_logPrefix Attempted to send frame (type ${frame.type}) on closed session. Allowed only for GO_AWAY.',
        );
        throw StateError(
      ...
      _log.warning(
        '$_logPrefix Attempted to send frame (type ${frame.type}) during/after cleanup. Suppressing.',
      );
    ...
    try {
      _log.fine(
        '$_logPrefix SEND: Type=${frame.type}, StreamID=${frame.streamId}, Flags=${frame.flags}, Length=${frame.length}',
      );
      ...
      if (frame.streamId == 1 && (frame.flags & YamuxFlags.syn != 0)) {
        ...
        _log.warning(
          '$_logPrefix SENDING IDENTIFY SYN (StreamID 1): Type=${frame.type}, Flags=0x${frame.flags.toRadixString(16).padLeft(2, '0')}, Length=${frame.length}, Bytes: $bytes',
        );
      } else if (frame.type == YamuxFrameType.newStream &&
          (frame.flags & YamuxFlags.syn != 0)) {
        // Log other SYN frames too for context
        final bytes = frame.toBytes();
        _log.fine(
          '$_logPrefix SENDING SYN (StreamID ${frame.streamId}): Type=${frame.type}, Flags=0x${frame.flags.toRadixString(16).padLeft(2, '0')}, Length=${frame.length}, Bytes: $bytes',
        );
      }
  ...
  Future<void> _goAway(YamuxCloseReason reason) async {
    if (_closed && _cleanupStarted) {
      _log.fine(
        '$_logPrefix _goAway(${reason.name}) called, but session already closed and cleanup started/finished.',
      );
      return;
    }
    if (_closed && !_cleanupStarted) {
      _log.warning(
        '$_logPrefix _goAway(${reason.name}) called when _closed=true but _cleanupStarted=false. Ensuring cleanup.',
      );
      ...
      _closed = true;
      _log.fine(
        '$_logPrefix _goAway(${reason.name}): Attempting to send GO_AWAY. Session marked _closed=true.',
      );
    }
    ...
    try {
      if (!_connection.isClosed && !_cleanupStarted) {
        ...
        await _sendFrame(frame);
        _log.fine(
          '$_logPrefix _goAway(${reason.name}): GO_AWAY frame send attempt complete.',
        );
      } else {
        _log.fine(
          '$_logPrefix _goAway(${reason.name}): Underlying connection closed or cleanup started. Skipping GO_AWAY frame send.',
        );
      }
    } catch (e) {
      _log.warning(
  ...
  Future<core_mux.MuxedStream> openStream(Context context) async {
    _log.fine(
      '$_logPrefix openStream: START for outgoing stream. Next Stream ID: $_nextStreamId. Context HashCode: ${context.hashCode}',
    ); // Elevated log level
    ...
    _log.finer(
      '$_logPrefix YamuxSession.newStream: Assigned streamId $streamId. Next will be $_nextStreamId.',
    );
    ...
    _log.finer(
      '$_logPrefix YamuxSession.newStream: YamuxStream for ID $streamId instantiated and added to _streams map.',
    );

    final completer = Completer<void>();
    _pendingStreams[streamId] = completer;
    _log.finer(
      '$_logPrefix YamuxSession.newStream: Added pending stream completer for ID $streamId.',
    );

    try {
      _log.fine(
        '$_logPrefix openStream: Attempting to send SYN for new stream ID $streamId.',
      ); // Elevated log level
      final frame = YamuxFrame.newStream(streamId);
      _log.fine(
        '$_logPrefix openStream: Created Frame for SYN: Type=${frame.type}, StreamID=${frame.streamId}, Flags=${frame.flags}, Length=${frame.length}',
      ); // ADDED LOG
      await _sendFrame(frame);
      _log.fine(
        '$_logPrefix openStream: Successfully sent SYN for new stream ID $streamId.',
      ); // Elevated log level

      _log.fine(
        '$_logPrefix openStream: Waiting for SYN-ACK for stream ID $streamId (timeout: ${_config.streamWriteTimeout}).',
      ); // Elevated log level
      await completer.future.timeout(_config.streamWriteTimeout);
      _log.fine(
        '$_logPrefix openStream: Received SYN-ACK for stream ID $streamId.',
      ); // Elevated log level

      _log.finer(
        '$_logPrefix openStream: Calling stream.open() for ID $streamId.',
      );
      await stream.open();
      _log.fine(
        '$_logPrefix openStream: Outgoing YamuxStream ID $streamId opened locally.',
      ); // Elevated log level
      return stream;
    } catch (e) {
      _log.severe(
        '$_logPrefix openStream: FAILED to open new stream ID $streamId: $e',
      ); // More specific message
      _pendingStreams.remove(streamId);
  ...
  Future<void> close() async {
    _log.fine(
      '$_logPrefix close() called. Initial _closed state: $_closed, _cleanupStarted: $_cleanupStarted',
    );
    if (_closed && _cleanupStarted) {
      _log.fine(
        '$_logPrefix close(): Session already closed and cleanup started/finished.',
      );
      return;
    }
  ...
  Future<void> _cleanupWithoutFrames() async {
    if (_cleanupStarted) {
      _log.fine(
        '$_logPrefix _cleanupWithoutFrames: Cleanup already started or completed.',
      );
      return;
    }
    ...
    _closed = true;
    _log.fine(
      '$_logPrefix _cleanupWithoutFrames: Starting cleanup. _closed=true, _cleanupStarted=true.',
    );
    ...
    _pendingStreams.forEach((id, completer) {
      if (!completer.isCompleted) {
        _log.finer(
          '$_logPrefix _cleanupWithoutFrames: Failing pending stream ID $id.',
        );
    ...
    _streams.clear();
    _log.finer(
      '$_logPrefix _cleanupWithoutFrames: Active streams map cleared. Processing ${activeStreams.length} streams for reset.',
    );
    for (final stream in activeStreams) {
      try {
        _log.finer(
          '$_logPrefix _cleanupWithoutFrames: Forcibly resetting stream ${stream.id()}.',
        );
    ...
    try {
      if (!_incomingStreamsController.isClosed) {
        ...
        _log.finer(
          '$_logPrefix _cleanupWithoutFrames: Incoming streams controller closed.',
        );
      }
    } catch (e) {
      _log.warning(
    ...
    try {
      if (!_connection.isClosed) {
        ...
        _log.finer(
          '$_logPrefix _cleanupWithoutFrames: Underlying connection closed.',
        );
      }
    } catch (e) {
      _log.warning(
