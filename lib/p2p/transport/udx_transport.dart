...
import 'dart:io'
    show
        ...
        RawDatagramSocket,
        Socket,
        SocketException; // For specific error types and network classes
import 'dart:math'; // Added import for Random
import 'dart:typed_data';

import 'package:dart_libp2p/core/connmgr/conn_manager.dart'; // For ConnManager class
import 'package:dart_libp2p/core/crypto/keys.dart';
import 'package:dart_libp2p/core/multiaddr.dart';
import 'package:dart_libp2p/core/network/common.dart' show Direction;
import 'package:dart_libp2p/core/network/conn.dart'
    show ConnState, ConnStats, Stats; // Added Stats
import 'package:dart_libp2p/core/network/context.dart' show Context;
import 'package:dart_libp2p/core/network/mux.dart' show MuxedConn, MuxedStream;
import 'package:dart_libp2p/core/network/rcmgr.dart' show ConnScope;
import 'package:dart_libp2p/core/network/stream.dart' show P2PStream;
import 'package:dart_libp2p/core/network/transport_conn.dart';
import 'package:dart_libp2p/core/peer/peer_id.dart' show PeerId;
import 'package:dart_libp2p/p2p/transport/connection_state.dart'; // Correct import for ConnectionState enum
...
class UDXTransport implements Transport {
  ...
  Future<TransportConn> dial(MultiAddr addr, {Duration? timeout}) async {
    ...
    return UDXExceptionHandler.handleUDXOperation(
  ...
  Future<TransportConn> _performDial(
    ...
    try {
      ...
      final random = Random();
      final localInitialStreamId =
          random.nextInt(0xFFFFFFFF) + 1; // Ensure non-zero
      var remoteInitialStreamId = random.nextInt(0xFFFFFFFF) + 1;
      ...
      final localMa = MultiAddr(
        '/ip4/${rawSocket.address.address}/udp/${rawSocket.port}/udx',
      );
      final remoteMa = addr;
...
class UDXSessionConn implements MuxedConn, TransportConn {
  UDXSessionConn({
    ...
    _initialStreamCloseSubscription = _initialStream.closeEvents.listen(
      (_) {
        _logger.fine(
          '[UDXSessionConn $id] Initial stream closed event. Closing session.',
        );
        close();
      },
