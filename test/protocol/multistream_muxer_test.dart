...
class MockConnStats implements ConnStats {
  ...
  final Stats stats;
  @override
  final int numStreams; // Removed const
}
...
class MockConn implements Conn {
  ...
  PeerId get localPeer => PeerId.fromString(
        'QmSoLnSGccFuZQJzRadHn95W2CrSFmGLwDsTU6gEdKnHv2',
      ); // Example valid PeerID
  ...
  PeerId get remotePeer => PeerId.fromString(
        'QmSoLSgciZgHiZ8isU3g2mQ3z5dSg2YV6x2v2tTjK2Yx8a',
      ); // Example valid PeerID
...
class MockP2PStream implements P2PStream {
  ...
  Future<void> reset() async {
    final err = StateError(
      'Stream reset',
    ); // This error is for the other side of the pipe.
    if (!_localCloseCompleter.isCompleted) {
      _localCloseCompleter.complete();
    }
    if (!_remoteCloseCompleter.isCompleted) {
    ...
    if (_pendingReadCompleter != null && !_pendingReadCompleter!.isCompleted) {
      ...
      ); // Or a more specific "Stream reset during read"
      _pendingReadCompleter = null;
    }
  ...
  group('MultistreamMuxer', () {
    ...
    test('protocol negotiation succeeds', () async {
      ...
      final (negotiatedProto, _) =
          await serverNegotiation.future.timeout(const Duration(seconds: 2));
      ...
      (() async {
        try {
          await muxerA.negotiate(
            streamA,
          ); // Server will negotiate, but client offers nothing it supports
        } catch (e) {
    ...
    test('addHandler overrides existing handler for the same protocol name',
      ...
      await serverNegotiation.future.timeout(const Duration(seconds: 1));
