...
void main() {
  group('MultistreamMuxer with Noise Handshake', () {
    ...
    setUp(() async {
      ...
      clientPeerId =
          core_peer.PeerId.fromPublicKey(clientIdentityKey.publicKey);
      clientNoiseSec = await NoiseSecurity.create(clientIdentityKey);

      serverIdentityKey = await generateEd25519KeyPair();
      serverPeerId =
          core_peer.PeerId.fromPublicKey(serverIdentityKey.publicKey);
    ...
    test(
      ...
      final clientSecuredConn =
          await clientNoiseSec.secureOutbound(adapterFromClient);

      // Wait for server-side handshake to complete
      final serverSecuredConn = await serverHandshakeCompleter.future.timeout(
        const Duration(seconds: 10), // Increased timeout for handshake
        onTimeout: () =>
