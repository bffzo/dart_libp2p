...
void main() {
  group('Multistream Timeout Reproduction Tests', () {
    test('reproduces the exact 30-second timeout error from client app',
      ...
      final hangingStream = HangingMockStream();
    ...
    test('demonstrates timer exception handling problem', () async {
      ...
      var unhandledExceptionOccurred = false;
    ...
    test('timeout during protocol negotiation phase', () async {
      ...
      expect(
        () async => muxer.negotiate(hangingStream),
    ...
    test('timeout with retry configuration', () async {
      ...
      expect(
        () async => muxer.selectOneOf(hangingStream, ['/test/protocol']),
        throwsA(isA<TimeoutException>()),
      );
