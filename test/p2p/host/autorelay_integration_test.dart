  ...
  group('AutoRelay Integration', () {
    ...
    setUp(() async {
      ...
      final relayServerAddrs = relayDirectAddrs.map((addr) {
        return '$addr/p2p/${relayPeerId.toBase58()}';
      }).toList();
      ...
      const autoNATConfig = AmbientAutoNATv2Config(
      ...
      autoRelaySubA = autoRelaySubASub.stream.listen((event) {
        if (event is EvtAutoRelayAddrsUpdated) {
          ...
          for (final addr in event.advertisableAddrs) {
            print('   - $addr');
            print(
    ...
    test(
      'Peers advertise circuit relay addresses and can ping through relay',
      () async {
        ...
        expect(
          peerAHost.network.connectedness(relayPeerId).name,
          equals('connected'),
        );
        expect(
          peerBHost.network.connectedness(relayPeerId).name,
          equals('connected'),
        );
        print(
        ...
        for (final addr in relayKnownPeerAAddrs) {
          print('   - $addr');
        }
        ...
        for (final addr in relayKnownPeerBAddrs) {
          print('   - $addr');
        }
        ...
        if (existingPeerBAddrs.isEmpty) {
          ...
          for (final addr in existingPeerBAddrs) {
            print('   - $addr');
        ...
        expect(
          storedCircuitAddrs.length,
          equals(peerBDialableCircuitAddrs.length),
          reason:
    ...
    ); // Increased timeout for teardown
  });
}
