...
void main() {
  group('MemoryPeerstore', () {
    ...
    test(
      'direct addrBook.addrs() call should not hang after addAddrs',
      () async {
        ...
        print('🔧 Adding addresses for peer $targetPeerId...');

        // Step 1: Add addresses (mimics /connect endpoint)
        await peerstore.addrBook
            .addAddrs(targetPeerId, targetAddrs, const Duration(hours: 1));

        print(
          '✅ Added ${targetAddrs.length} addresses for peer $targetPeerId',
        );

        // Step 2: Look up addresses (mimics holepunch handler)
        print(
          '🔎 Looking up addresses for peer $targetPeerId in peerstore...',
        );
        ...
        print(
          '🔎 Found ${existingAddrs.length} addresses for peer $targetPeerId',
        );

        // If we get here, the lookup worked
        expect(
          existingAddrs.length,
          equals(2),
          reason: 'Should find both addresses that were added',
        );
        expect(
          existingAddrs,
          containsAll(targetAddrs),
    ...
    test(
      ...
      () async {
        ...
        print(
          '🔧 Adding addresses for target peer $targetPeerId...',
        );
        ...
        for (var i = 0; i < 5; i++) {
          futures.add(
            Future.delayed(Duration(milliseconds: i * 10), () async {
              ...
              final addrs =
                  await isolatedPeerstore.addrBook.addrs(targetPeerId);
              print(
                '🔎 Lookup $i: Found ${addrs.length} addresses for peer $targetPeerId',
              );
              return addrs;
            }),
          );
