...
class MockStunServer {
  ...
  void _handleRequest(Datagram datagram) {
    ...
    final addressBytes =
        mappedAddress.address.split('.').map(int.parse).toList();
    ...
    if (_otherAddress != null && _otherPort != null) {
      ...
      final otherIpBytes =
          _otherAddress!.address.split('.').map(int.parse).toList();
  ...
  void _handleChangeRequest(
    StunMessage request,
    Datagram datagram,
    Uint8List changeRequestData,
  ) {
    if (changeRequestData.length < 4) return;

    final buffer = ByteData.view(
      changeRequestData.buffer,
      changeRequestData.offsetInBytes,
    );
