  ...
  group('NoiseXXPattern', () {
    ...
    test('verifies different static keys produce different shared secrets',
      ...
      expect(
        ...
        throwsA(
          isA<SecretBoxAuthenticationError>().having(
            (e) => e.toString(),
            'toString()',
    ...
    test('key derivation hmac inputs follow noise specification', () async {
      ...
      expect(
        initiatorSendKey1,
        equals(initiatorSendKey2),
        reason: 'Send key should be stable',
      );
      expect(
        initiatorRecvKey1,
        equals(initiatorRecvKey2),
      ...
      expect(
        initiatorSendKey1,
        isNot(equals(initiatorRecvKey1)),
    ...
    test('key derivation follows noise specification', () async {
      ...
      expect(
        initiatorSendKey.length,
        equals(32),
        reason: 'Keys should be 32 bytes for ChaCha20',
      );
      expect(
        responderSendKey.length,
        equals(32),
        reason: 'Keys should be 32 bytes for ChaCha20',
      );
    ...
    test('minimal dh test', () async {
      ...
      expect(
        initiator.debugChainKey,
        equals(responder.debugChainKey),
    ...
    test('proves hash mixing order issue', () async {
      ...
      expect(
        initiator.debugHandshakeHash,
        equals(responder.debugHandshakeHash),
    ...
    test('proves payload is encrypted in final message', () async {
      ...
      var hasPayloadPattern = false;
      for (var i = 0; i < message3.length - 3; i++) {
        if (message3[i] == 0x42 &&
            message3[i + 1] == 0x42 &&
            message3[i + 2] == 0x42) {
    ...
    test('readMessage correctly parses final message with static key and MAC',
      ...
      expect(
        message3.length,
        equals(48),
        reason: 'Final message should be 48 bytes (32 encrypted key + 16 MAC)',
      );
    ...
    Future<Uint8List> encryptWithKey(Uint8List data, SecretKey key) async {
    ...
    Future<Uint8List> decryptWithKey(Uint8List data, SecretKey key) async {
      final algorithm = Chacha20.poly1305Aead();
      final nonce = List<int>.filled(algorithm.nonceLength, 0);
      const macSize = 16;
    ...
    test('proves handshake payload handling issue', () async {
      ...
      expect(
        responder.state,
        equals(XXHandshakeState.complete),
      ...
      final encryptedTestData =
          await encryptWithKey(testData, initiator.sendKey);
      final decryptedTestData =
          await decryptWithKey(encryptedTestData, responder.recvKey);
    ...
    test('follows Noise XX pattern specification exactly', () async {
      ...
      final initiatorEphemeralBytes = initiatorEphemeralPub.bytes;
      expect(
        message1,
        equals(initiatorEphemeralBytes),
      ...
      await responder.readMessage(message1);
      expect(
        responder.debugChainKey,
        equals(chainKeyBeforeE),
      ...
      final responderEphemeralBytes = responderEphemeralPub.bytes;
      expect(
        message2.sublist(0, 32),
        equals(responderEphemeralBytes),
        reason:
    ...
    test('proves correct dh operation order', () async {
      ...
      expect(
        initiator.debugChainKey,
        isNot(equals(chainKeyBeforeFinalMsg)),
      ...
      final initiatorStaticBytes = initiatorStaticPub.bytes;
      expect(
        responder.debugRemoteStaticKey,
        equals(initiatorStaticBytes),
    ...
    test('Step 1: -> e - initiator sends ephemeral key', () async {
      ...
      expect(
        initiatorInitialHash,
        equals(responderInitialHash),
        reason: 'Initial handshake hashes should match',
      );
    ...
    test(
      ...
      expect(
        initiator.state,
        equals(XXHandshakeState.sentEES),
      ...
      expect(
        initiator.debugChainKey,
        isNot(equals(chainKeyBeforeSecondMsg)),
      ...
      final responderStaticBytes = responderStaticPub.bytes;
      expect(
        initiator.debugRemoteStaticKey,
        equals(responderStaticBytes),
