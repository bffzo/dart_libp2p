...
void main() {
  group('TCPTransport Raw Connection Tests', () {
    ...
    setUp(() async {
      ...
      clientConn = results[0];
      serverConn = results[1];
    });
    ...
    test(
      ...
      expect(
        totalBytesRead,
        equals(dataToSend.length),
        reason: 'Should read the exact number of bytes sent.',
      );
      expect(
        receivedBuffer.sublist(0, totalBytesRead),
        orderedEquals(dataToSend),
    ...
    group('Yamux over TCP Tests', () {
      ...
      setUp(() async {
        ...
        yamuxClientTransport = TCPTransport(
          resourceManager: yamuxResourceManager,
          connManager: connManager,
        );
        ...
        clientTcpConnForYamux = results[0];
        serverTcpConnForYamux = results[1];

        const multiplexerConfig = p2p_mux.MultiplexerConfig(
