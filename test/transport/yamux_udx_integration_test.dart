...
void main() {
  ...
  group('Yamux-UDX Integration Test Suite', () {
      ...
      test(
        ...
        () async {
          ...
          try {
            ...
            clientTransport = UDXTransport(
              connManager: connManager,
              udxInstance: udxInstance,
            );
            serverTransport = UDXTransport(
              connManager: connManager,
              udxInstance: udxInstance,
            );
            ...
            final serverAcceptFuture = listener.accept().then((conn) {
              ...
              serverRawConn = conn;
              print(
                '   Server accepted raw UDX connection: ${serverRawConn.id}',
              );
            ...
            const yamuxConfig = MultiplexerConfig(
            ...
            // Open client stream
            clientStream = await clientSession
            ...
            try {
              if (!clientStream.isClosed) {
                await clientStream.close().timeout(const Duration(seconds: 2));
              }
              if (!serverStream.isClosed) {
                await serverStream.close().timeout(const Duration(seconds: 2));
              }
              if (!clientSession.isClosed) {
                await clientSession.close().timeout(const Duration(seconds: 2));
              }
              if (!serverSession.isClosed) {
                await serverSession.close().timeout(const Duration(seconds: 2));
              }
      ...
      test(
        ...
        () async {
          ...
          try {
            ...
            // Verify upgrade
            expect(
              clientUpgradedConn.remotePeer.toString(),
              serverPeerId.toString(),
            );
            expect(
              serverUpgradedConn.remotePeer.toString(),
            ...
            expect(
              clientUpgradedConn.state.streamMultiplexer,
              contains('yamux'),
            );
            expect(
              serverUpgradedConn.state.streamMultiplexer,
              contains('yamux'),
            );
            ...
            try {
              ...
              if (!clientUpgradedConn.isClosed) {
                await clientUpgradedConn
                    .close()
                    .timeout(const Duration(seconds: 2));
              }
              if (!serverUpgradedConn.isClosed) {
                await serverUpgradedConn
                    .close()
                    .timeout(const Duration(seconds: 2));
              }
              if (!listener.isClosed) {
    ...
    group('Layer 3: Yamux over UDX + Noise + BasicHost (Simple Protocol)', () {
      test(
        'should handle large payloads (100KB) over UDX + Noise + BasicHost with simple echo protocol',
        () async {
          print(
            '\n🧪 TEST 3: Yamux + UDX + Noise + BasicHost (Simple Echo Protocol)',
          );
          print(
            '   Goal: Test the full stack with a simple protocol to verify integration',
          );
          ...
          try {
            ...
            final resourceManager =
                ResourceManagerImpl(limiter: FixedLimiter());
            ...
            serverHost.setStreamHandler(echoProtocolId,
                (core_network_stream.P2PStream stream, PeerId peerId) async {
              print(
                '   Server Host received echo stream: ${stream.id()} from $peerId',
              );
              if (!serverStreamCompleter.isCompleted) {
            ...
            print(
              '   🚀 Starting large payload test (100KB) over BasicHost...',
            );
            ...
            try {
              ...
              if (clientHost != null) {
                await clientHost.close().timeout(const Duration(seconds: 5));
              }
              if (serverHost != null) {
                await serverHost.close().timeout(const Duration(seconds: 5));
              }
            } catch (e) {
...
Future<void> _testLargePayloadTransfer(
  ...
  await writeCompleter.future.timeout(const Duration(seconds: 30));
