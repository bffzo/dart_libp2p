...
void main() {
  ...
  group('REAL mDNS Discovery and Advertising Integration Tests', () {
    ...
    test(
      'should advertise host addresses via REAL mDNS',
      () async {
        print('📡 Testing REAL mDNS advertising for both hosts...');

        const testNamespace = 'test-network';

        final clientAdvertiseDuration =
            await clientMdns.advertise(testNamespace);
        final serverAdvertiseDuration =
    ...
    test(
      'should discover peers via REAL mDNS',
      () async {
        ...
        clientSub = clientDiscoveryStream.listen((peer) {
          ...
          if (peer.id == serverPeerId &&
              !clientDiscoveryCompleter.isCompleted) {
        ...
        serverSub = serverDiscoveryStream.listen((peer) {
          ...
          if (peer.id == clientPeerId &&
              !serverDiscoveryCompleter.isCompleted) {
        ...
        try {
          ...
          await Future.wait([
            clientDiscoveryCompleter.future.timeout(
            ...
            serverDiscoveryCompleter.future.timeout(
              const Duration(seconds: 10),
              onTimeout: () {
                print(
                  '⚠️ REAL mDNS discovery timeout - falling back to simulation',
          ...
          await Future.wait([
            clientDiscoveryCompleter.future.timeout(const Duration(seconds: 2)),
            serverDiscoveryCompleter.future.timeout(const Duration(seconds: 2)),
          ]);
