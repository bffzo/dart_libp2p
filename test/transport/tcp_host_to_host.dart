  ...
  */
  ...
  group('Host-to-Host TCP, Noise, Yamux, Ping (via BasicHost)', () {
    ...
    setUpAll(() async {
      ...
      clientPeerId =
          core_peer_id_lib.PeerId.fromPublicKey(clientKeyPair.publicKey);
      serverPeerId =
          core_peer_id_lib.PeerId.fromPublicKey(serverKeyPair.publicKey);
      ...
      final clientHostConfig = p2p_config.Config()
        ...
        ..negotiationTimeout =
            const Duration(seconds: 20) // For BasicHost protocol negotiation
      ...
      final serverHostConfig = p2p_config.Config()
        ...
        ..connManager = connManager
        ..addrsFactory = passThroughAddrsFactory
        ..negotiationTimeout = const Duration(seconds: 20)
      ...
      expect(
        serverHost.addrs.isNotEmpty,
        isTrue,
        reason: 'Server host should have listen addresses after start().',
      ); // Use getter
      serverListenAddr = serverHost.addrs.firstWhere(
        // Use getter
        (addr) => addr.hasProtocol(
          multiaddr_protocol.Protocols.tcp.name,
        ), // Changed to TCP
        orElse: () => throw StateError(
          'No TCP listen address found for server host',
        ),
      ); // Changed to TCP
      ...
      print(
        'Host-to-Host Setup Complete. Client: $clientPeerId, Server: $serverPeerId listening on $serverListenAddr',
      );
    ...
    test(
      ...
      () async {
        const testProtocolID = '/test-ping/1.0.0';
        final serverStreamCompleter =
            Completer<core_network_stream.P2PStream>();
        final serverHandlerFinishedProcessing = Completer<void>();

        print(
          'Server Host ($serverPeerId) setting stream handler for $testProtocolID',
        );
        ...
        final serverAddrInfo = AddrInfo(
          serverPeerId,
          serverHost.addrs,
        ); // Positional arguments, use getter for addrs
        print(
          'Client Host ($clientPeerId) connecting to Server Host ($serverPeerId) at $serverAddrInfo',
        );
        ...
        await clientHost.connect(serverAddrInfo).timeout(
        ...
        print(
          'Client Host connect call completed for $serverPeerId',
        );
        ...
        expect(
          clientNetwork.connsToPeer(serverPeerId).isNotEmpty,
          isTrue, // Correct method name
          reason: 'Client should have a connection to server after connect()',
        );

        print(
          'Client Host ($clientPeerId) opening new stream to $serverPeerId for $testProtocolID',
        );
        ...
        await serverHandlerFinishedProcessing.future.timeout(
          const Duration(seconds: 15),
          onTimeout: () {
            // Increased timeout
    ...
    ); // Increased overall test timeout
  });
}
