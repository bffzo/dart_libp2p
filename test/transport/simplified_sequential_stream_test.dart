...
void main() {
  ...
  Logger.root.onRecord.listen((record) {
    print(
      r'${record.level.name}: ${record.time}: ${record.loggerName}: ${record.message}',
    );
  ...
  group('Simplified Sequential Stream Test (TCP -> Noise -> Yamux)', () {
    ...
    setUp(() async {
      ...
      testLog.info(r'Server listening on: $serverListenAddr');
      ...
      testLog.info(
        r'Raw TCP connection established. Client: ${rawClientConn?.id}, Server: ${rawServerConn?.id}',
      );
      ...
      testLog.info(
        r'Connections upgraded. Client: ${upgradedClientConn?.state.security}/${upgradedClientConn?.state.streamMultiplexer}, Server: ${upgradedServerConn?.state.security}/${upgradedServerConn?.state.streamMultiplexer}',
      );
    ...
    tearDown(() async {
      ...
      await upgradedClientConn?.close().catchError(
            (e) => testLog.warning(r'Error closing upgradedClientConn: $e'),
          );
      await upgradedServerConn?.close().catchError(
            (e) => testLog.warning(r'Error closing upgradedServerConn: $e'),
          );
    ...
    test('Open, use, and close first stream, then open and use second stream',
      ...
      final clientStream1 = await clientStream1Future
          .timeout(const Duration(seconds: 5)); // Explicit type P2PStream
      testLog.info(
        r'Client stream 1 opened. ID (Yamux): ${(clientStream1 as YamuxStream).id()}',
      ); // Cast to YamuxStream for id()
      final serverStream1 = await serverStream1Future
          .timeout(const Duration(seconds: 5)); // Explicit type
      testLog.info(
        r'Server stream 1 accepted. ID (Yamux): ${(serverStream1 as YamuxStream).id()}',
      ); // Cast to YamuxStream for id()
      ...
      expect(
        ...
        isTrue,
        reason: 'Server should read EOF after client closes write on stream 1',
      );
      ...
      final clientStream2 = await clientStream2Future
          .timeout(const Duration(seconds: 5)); // Explicit type P2PStream
      testLog.info(
        r'Client stream 2 opened. ID (Yamux): ${(clientStream2 as YamuxStream).id()}',
      ); // Cast to YamuxStream for id()
      final serverStream2 = await serverStream2Future
          .timeout(const Duration(seconds: 5)); // Explicit type
      testLog.info(
        r'Server stream 2 accepted. ID (Yamux): ${(serverStream2 as YamuxStream).id()}',
      ); // Cast to YamuxStream for id()
      ...
      try {
        ...
        fail(r'Second stream operation failed: $e');
      } finally {
        testLog.info('Closing second stream...');
        await clientStream2.close().catchError(
              (e) => testLog.warning(r'Error closing clientStream2: $e'),
            );
        await serverStream2.close().catchError(
              (e) => testLog.warning(r'Error closing serverStream2: $e'),
            );
