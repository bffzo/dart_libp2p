...
void main() {
  group('SwarmStream and UDXP2PStreamAdapter Interaction', () {
    ...
    Future<
      ...
      final serverConn =
          await serverConnCompleter.future.timeout(const Duration(seconds: 5));
      ...
      final serverStream =
          await serverStreamFuture.timeout(const Duration(seconds: 5));
    ...
    test(
      'should handle large data transfer (client to server)',
      () async {
        ...
        final clientConn =
            await clientSwarm.dialPeer(core_context.Context(), serverPeerId);
        final serverConn = await serverConnCompleter.future
        ...
        try {
          ...
          await serverAcceptStreamFuture.timeout(const Duration(seconds: 5));
    ...
    test('should handle stream reset correctly', () async {
      ...
      try {
        ...
        await expectLater(
          () => clientStream.rawWrite(Uint8List(1)),
          throwsA(isA<StateError>()),
        );

        await serverReadFuture.timeout(const Duration(seconds: 5));
      } finally {
  ...
  group('Stream EOF and Best Practices Demonstration', () {
    ...
    Future<
      ...
      await serverAcceptFuture.timeout(const Duration(seconds: 5));
    ...
    test('EOF Detection Patterns - Graceful closeWrite()', () async {
      ...
      try {
        ...
        final serverReadFuture = () async {
          ...
          final data = await StreamEOFUtils.readUntilEOF(
            serverStream,
            context: 'Server',
          );
    ...
    test('Chunked Data Reading with EOF', () async {
      ...
      try {
        ...
        final serverReadFuture = () async {
          ...
          final allData = await StreamEOFUtils.readUntilEOF(
            serverStream,
            context: 'Server',
          );
        ...
        await serverReadFuture.timeout(const Duration(seconds: 15));
    ...
    test('Protocol-Style Reading (Length-Prefixed)', () async {
      ...
      try {
        ...
        final lengthPrefix = ByteData(4)..setUint32(0, message.length);
        ...
        final serverReadFuture = () async {
          ...
          final messageLength = ByteData.view(lengthBytes.buffer).getUint32(0);
    ...
    test('Timeout and Error Recovery Patterns', () async {
      ...
      try {
        ...
        final serverReadFuture = () async {
          ...
          final data = await StreamEOFUtils.safeRead(
            serverStream,
            context: 'Server-Real',
          );
          expect(data, equals(testData));
