...
void main() {
  ...
  group('TCPConnection with Real Sockets', () {
    ...
    final serverReceivedData = <Uint8List>[];

    const localHostIp = '127.0.0.1';
    var port = 0; // Will be set by ServerSocket
    ...
    setUp(() async {
      ...
      final localClientMultiaddr = MultiAddr(
        '/ip4/${clientSocket.address.address}/tcp/${clientSocket.port}',
    ...
    test('Server sends multiple small chunks, client reads them sequentially',
      ...
      final chunk2 = Uint8List.fromList(
        List.generate(30, (i) => i + 10),
      ); // 30 bytes, "Nonce 4" size
      final chunk3 = Uint8List.fromList(
        List.generate(30, (i) => i + 100),
      ); // 30 bytes, "Nonce 5" size
    ...
    test(
      ...
      const chunkPartialExpectedLength =
      ...
      expect(
        eofRead,
        isEmpty,
        reason: 'Subsequent read after partial receive and close should be EOF',
      );
      print('Test: Client received EOF as expected.');
    });

    test('Client read times out if server sends no data', () async {
      const readTimeoutDuration = Duration(milliseconds: 100);
