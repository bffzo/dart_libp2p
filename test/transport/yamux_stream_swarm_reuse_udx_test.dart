...
void main() {
  ...
  group('YamuxStream Connection Re-use via UDX Transport', () {
    ...
    test(
      'swarm streams reuse yamux connections correctly with UDX transport',
      () async {
        print(
        ...
        try {
          ...
          expect(
            connectionsAfterPause.length,
            equals(1),
            reason: 'Should still have one connection after pause',
          );
          expect(
            connectionsAfterPause.first.id,
            equals(sharedConn.id),
          ...
          for (var i = 0; i < 2; i++) {
            ...
            final newStream =
                await swarmB.newStream(Context(), swarmA.localPeer);
          ...
          expect(
            connectionsAfter.length,
            equals(1),
            reason: 'Should still have only one connection',
          );
          expect(
            connectionsAfter.first.id,
            equals(sharedConn.id),
          ...
          expect(
            finalConnections.first.id,
            equals(sharedConn.id),
            reason: 'Should be the original connection throughout',
          );
          expect(
            finalConnections.first.isClosed,
            isFalse,
          ...
          print(
            '✓ Total streams created: ${streams.length + newStreams.length}',
          );
          print('✓ Streams closed: 2');
          print(
            '✓ Streams remaining: ${streams.length + newStreams.length - 2}',
          );
        ...
        final serverNotifiee = TestNotifiee(
          connectedCallback: (network, conn) {
            if (conn.remotePeer.toString() == swarmB.localPeer.toString() &&
              ...
              print(
                'Server received connection from client ${conn.remotePeer}',
              );
    ...
    test('large data transfer with connection reuse', () async {
      ...
      try {
        ...
        for (final size in testSizes) {
          ...
          await serverAcceptFuture.timeout(const Duration(seconds: 10));
      ...
      try {
        ...
        final stream =
            await ((serverConn as dynamic).conn as core_mux_types.MuxedConn)
                .acceptStream()
                .timeout(const Duration(milliseconds: 500)) as P2PStream;
