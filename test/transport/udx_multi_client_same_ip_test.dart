...
void main() {
  ...
  group('UDX Transport: Multiple Clients from Same IP, Different Ports', () {
    ...
    setUpAll(() async {
      ...
      client1PeerId =
          core_peer_id_lib.PeerId.fromPublicKey(client1KeyPair.publicKey);
      client2PeerId = core_peer_id_lib.PeerId.fromPublicKey(
        client2KeyPair.publicKey,
      ); // Re-enable client2
      ...
      client2Host.peerStore.addrBook.addAddrs(
        serverPeerId,
        [serverListenAddr],
        AddressTTL.permanentAddrTTL,
      ); // Re-enable client2
    ...
    test(
      ...
      () async {
        ...
        const testProtocolID = '/multi-client-echo/1.0.0';
        ...
        serverHost.setStreamHandler(testProtocolID, (stream, remotePeer) async {
          ...
          try {
            ...
            if (remotePeer == client2PeerId &&
              ...
              serverReceivedDataFromClient2
                  .completeError(e); // Re-enable client2
            }
          } finally {
        ...
        expect(
          serverConns.length,
          equals(2),
          reason: 'Server should have two connections.',
        ); // Restore for two clients

        final connsToClient1List =
            serverHost.network.connsToPeer(client1PeerId);
        final connToClient1 =
        ...
        expect(
          connToClient2,
          isNotNull,
          reason: 'Server should have a connection to Client2.',
        ); // Re-enable client2
        ...
        print(
          'Server: Conn to Client2 remote multiaddr: $remoteAddrClient2',
        ); // Re-enable client2
        ...
        expect(
          udpComponentClient1,
          isNot(equals(udpComponentClient2)),
          reason:
        ...
        final dataC2 = Uint8List.fromList(
          List.generate(
            24,
            (_) => random.nextInt(256),
          ),
        ); // Re-enable client2 data
        ...
        final serverReceivedC2 = await serverReceivedDataFromClient2.future
            .timeout(const Duration(seconds: 5)); // Re-enable client2
