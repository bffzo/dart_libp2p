...
void main() {
  ...
  group('UDXTransport with Noise and OBP Integration Test', () {
    ...
    test(
      ...
      () async {
        ...
        final clientUpgradedConn = upgradedConns[0];
        final serverUpgradedConn = upgradedConns[1];
        ...
        expect(
          handshakeSuccess,
          isTrue,
          reason: 'OBP handshake should succeed',
        );
    ...
    test(
      ...
      () async {
        ...
        clientStream = await (clientUpgradedConn as core_mux_types.MuxedConn)
            .openStream(core_context.Context()) as YamuxStream;
        print(
          'Client opened stream: ${clientStream.id()} (Real Managers Test)',
        );
        ...
        expect(pongResponse!.type, equals(OBPMessageType.pong));
        expect(
          utf8.decode(pongResponse.payload),
          equals('real-managers-test-data'),
        );
        ...
        print(
          'Closing client stream ${clientStream.id()} (Real Managers Test)',
        );
        await clientStream.close();
        print(
          'Closing server stream ${serverStream.id()} (Real Managers Test)',
        );
        await serverStream.close();
  ...
  group('Swarm with UDX, Noise, and OBP', () {
    ...
    setUpAll(() async {
      ...
      print(
        'Swarm UDX/Noise/OBP Setup Complete. Client: $clientPeerId, Server: $serverPeerId listening on $serverListenAddr',
      );
    });
    ...
    test(
      ...
      () async {
        print(
          'Client Host ($clientPeerId) attempting to open new stream to Server Host ($serverPeerId) for protocol ${OBPConstants.protocolId}',
        );
        ...
        expect(
          clientStream.conn.remotePeer.toString(),
          serverPeerId.toString(),
        );
...
Future<void> _testOBPFeatures(
  core_network_stream.P2PStream initialStream,
) async {
  ...
  try {
    ...
    expect(
      utf8.decode(pongResponse.payload),
      equals('comprehensive-test-ping'),
    );
  ...
  try {
    ...
    final largePongResponse = await OBPProtocolHandler.sendRequest(
      ...
      context: 'SwarmTest',
      timeout:
          const Duration(seconds: 60), // Increased timeout for large payload
    );
  ...
  try {
    ...
    for (var i = 0; i < 5; i++) {
      ...
      futures.add(
        OBPProtocolHandler.sendRequest(
          currentStream,
          rapidFrame,
          context: 'SwarmTest',
        ),
    ...
    for (var i = 0; i < rapidResponses.length; i++) {
      expect(
        rapidResponses[i],
        isNotNull,
