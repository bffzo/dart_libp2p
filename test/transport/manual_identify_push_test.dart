...
void main() {
  ...
  group('Manual Identify Push Test', () {
    ...
    setUp(() async {
      ...
      clientPeerId =
          concrete_peer_id.PeerId.fromPublicKey(clientKeyPair.publicKey);
      serverKeyPair = await crypto_ed25519.generateEd25519KeyPair();
      serverPeerId =
          concrete_peer_id.PeerId.fromPublicKey(serverKeyPair.publicKey);
        ...
        ..muxers = [
          config_stream_muxer.StreamMuxer(
            ...
            muxerFactory: (Conn secureConn, bool isClient) {
              const yamuxInternalConfig = p2p_mux.MultiplexerConfig();
              return YamuxSession(
      ...
      when(clientKeyBook.pubKey(clientPeerId))
          .thenAnswer((_) async => clientKeyPair.publicKey);
      when(clientKeyBook.privKey(clientPeerId))
          .thenAnswer((_) async => clientKeyPair.privateKey);
      when(clientKeyBook.pubKey(serverPeerId))
          .thenAnswer((_) async => serverKeyPair.publicKey); // For remote peer
      // Mock the specific addPubKey calls that will be made
      when(clientKeyBook.addPubKey(serverPeerId, serverKeyPair.publicKey))
          .thenAnswer((_) async {});
      when(clientKeyBook.addPubKey(clientPeerId, clientKeyPair.publicKey))
      ...
      when(mockClientMuxer.selectOneOf(any, argThat(equals([id]))))
        ...
        final protocols = invocation.positionalArguments[1]
      ...
      when(serverKeyBook.pubKey(serverPeerId))
          .thenAnswer((_) async => serverKeyPair.publicKey);
      when(serverKeyBook.privKey(serverPeerId))
          .thenAnswer((_) async => serverKeyPair.privateKey);
      when(serverKeyBook.pubKey(clientPeerId))
          .thenAnswer((_) async => clientKeyPair.publicKey); // For remote peer
      // Mock the specific addPubKey calls that will be made
      when(serverKeyBook.addPubKey(clientPeerId, clientKeyPair.publicKey))
          .thenAnswer((_) async {});
      when(serverKeyBook.addPubKey(serverPeerId, serverKeyPair.publicKey))
      ...
      clientPeerStore.keyBook
          .addPubKey(serverPeerId, serverKeyPair.publicKey); // Removed await
      serverPeerStore.keyBook
          .addPubKey(clientPeerId, clientKeyPair.publicKey); // Removed await

      // Add supported protocols for identify to peerstores
      clientPeerStore.protoBook.addProtocols(
        serverPeerId,
        [id, idPush],
      ); // Use direct constants, removed await
      serverPeerStore.protoBook.addProtocols(
        clientPeerId,
        [id, idPush],
      ); // Use direct constants, removed await
    ...
    tearDown(() async {
      ...
      await clientMuxedConn?.close().catchError(
            (e) => testLog.warning(r'Error closing clientMuxedConn: $e'),
          );
      await serverMuxedConn?.close().catchError(
            (e) => testLog.warning(r'Error closing serverMuxedConn: $e'),
          );
      await serverListener.close().catchError(
            (e) => testLog.warning(r'Error closing serverListener: $e'),
          );
      testLog.info('=== Test Teardown Complete: Manual Identify Push ===');
    });

    test(
      'Manual Identify Push sequence',
      () async {
        ...
        await clientIdentifyStream
            .setProtocol(clientSelectedProtocol!); // Set it on the stream
        ...
        await serverIdentifyStream
            .setProtocol(id); // Simulate server side has selected 'id'
        ...
        unawaited(
          clientIdentifyService.sendIdentifyResponse(
            clientIdentifyStream,
            false,
          ),
        ); // isPush = false
        ...
        testLog
            .info('Client handling initial identify response from server...');
        ...
        try {
          ...
          testLog
              .warning(r'Error reading server initial identify on client: $e');
        }
        ...
        try {
          ...
          testLog
              .warning(r'Error reading client initial identify on server: $e');
        }
        ...
        try {
          ...
          testLog.info(r'Server opened push stream: ${serverPushStream!.id()}');

          // Server sends its identify message on the push stream
          unawaited(
            serverIdentifyService.sendIdentifyResponse(
              serverPushStream!,
              true,
            ),
          ); // isPush = true
          ...
          testLog.severe(
            r'Error during server initiating identify push: $e',
            e,
            s,
          );
          fail(r'Server failed to initiate identify push: $e');
        }

        // Client accepts the push stream
        final clientPushStream =
            await clientAcceptPushFuture.timeout(const Duration(seconds: 5));
        expect(
          clientPushStream,
          isNotNull,
        ...
        testLog.info(
          r'Client accepted push stream: ${(clientPushStream as YamuxStream).id()}',
        );
        ...
        try {
          ...
          testLog.severe(
            r'MAC ERROR OR OTHER FAILURE: Server failed to read/decrypt client response on push stream: $e',
            e,
            s,
          );
          fail(
            r'Server failed to read/decrypt client response on push stream: $e',
          );
        } finally {
          await clientPushStream.close().catchError(
                (e) => testLog.warning(r'Error closing clientPushStream: $e'),
              );
          await serverPushStream.close().catchError(
                (e) => testLog.warning(r'Error closing serverPushStream: $e'),
              );
        }
    ...
    ); // Increased timeout for more complex interaction
  });
}
...
extension IdentifyServiceTestHelpers on IdentifyService {
  Future<P2PStream?> newStreamAndNegotiate(Conn conn, String protocol) async {
    ...
    final stream = await conn
        .newStream(Context()); // streamId 0 is arbitrary for this direct call
  ...
  Future<void> sendIdentifyResponse(P2PStream stream, bool isPush) async {
    ...
    await sendIdentifyResp(stream, isPush); // Use public method
  }
