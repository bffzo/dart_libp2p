...
void main() {
  ...
  group('UDX Transport Listening', () {
    ...
    test(
      'should listen on UDX and report correct multiaddresses',
      () async {
        ...
        for (final addr in reportedAddrs) {
        ...
        if (foundUdxAddr != null) {
          ...
          expect(
            foundUdxAddr.hasProtocol(Protocols.udx.name),
            isTrue,
          ...
          expect(
            foundUdxAddr.hasProtocol(Protocols.p2p.name),
            isFalse,
            reason: '/p2p component should not be in addresses from host.addrs',
          );
        }
      },
