...
void main() {
  group('Swarm with UDX, Noise, and HTTP', () {
    ...
    setUpAll(() async {
      ...
      print(
        'Swarm UDX/Noise/HTTP Setup Complete. Client: $clientPeerId, Server: $serverPeerId listening on $serverListenAddr',
      );
    });
    ...
    test(
      ...
      () async {
        print(
          'Client Host ($clientPeerId) connecting to Server Host ($serverPeerId) for HTTP protocol',
        );
    ...
    test(
      'should handle POST requests with JSON data',
      () async {
    ...
    test(
      'should handle custom headers and advanced HTTP features',
      () async {
        ...
        try {
          ...
          expect(
            protectedData['user_agent'],
            equals('dart-libp2p-test-client/1.0'),
          );
          ...
          final response4 = await httpClient.getRequest(
            serverPeerId,
            '/api/users/user123/posts/post456',
          );
    ...
    test(
      'should handle PUT and DELETE HTTP methods',
      () async {
    ...
    test(
      'should handle concurrent HTTP requests efficiently',
      () async {
        ...
        try {
          ...
          for (var i = 0; i < sequentialResults.length; i++) {
    ...
    test(
      'should handle HTTP error scenarios and edge cases',
      () async {
        ...
        try {
          ...
          expect(
            validResult!['message'],
            equals('Content validated successfully'),
          );
