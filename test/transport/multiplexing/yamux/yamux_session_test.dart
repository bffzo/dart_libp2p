  ...
  group('YamuxSession and YamuxStream Conn Integration', () {
    ...
    setUp(() async {
      ...
      clientPeerId = PeerId.fromBytes(
        Uint8List.fromList(
          List.generate(34, (i) => (i % 250) + 1)
            ..[0] = 0x12
            ..[1] = 0x20,
        ),
      ); // Example Ed25519 PeerId bytes
      serverPeerId = PeerId.fromBytes(
        Uint8List.fromList(
          List.generate(34, (i) => (i % 250) + 2)
            ..[0] = 0x12
            ..[1] = 0x20,
        ),
      ); // Example Ed25519 PeerId bytes
      ...
      when(mockTransportConn.state).thenReturn(
        const core_conn.ConnState(
      ...
      yamuxConfig = const MultiplexerConfig(
    ...
    tearDown(() async {
      ...
      if (!clientSession.isClosed) {
        ...
        when(mockTransportConn.isClosed).thenReturn(
          false,
        ); // Simulate not yet closed for session's close logic
    ...
    test(
      ...
      expect(
        yamuxStream.conn,
        isA<core_conn.Conn>(),
        reason: 'stream.conn should be a Conn object.',
      ); // Use core_conn.Conn for type check
      expect(
        yamuxStream.conn,
        same(clientSession),
        reason:
      ...
      when(mockTransportConn.isClosed).thenReturn(
        false,
      ); // Ensure it's not seen as already closed by stream logic
