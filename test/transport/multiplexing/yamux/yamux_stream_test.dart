...
class MockConnScope implements ConnScope {
  ...
  void setPeer(
    PeerId p,
  ) {} // This was in the mock, but not in ConnScope interface. Retaining for now.
  ...
  Future<void> addStream(
    P2PStream stream,
  ) async {} // Not in ConnScope interface
  @override
...
void main() {
  group('YamuxStream', () {
    ...
    test('opens stream correctly', () async {
      ...
      expect(
        sentFrames[0].data.buffer.asByteData().getUint32(0),
        equals(256 * 1024),
      );
    });
    ...
    group('flow control', () {
      ...
      test('updates window size on receiving window update', () async {
        ...
        const updateSize = 50 * 1024; // 50KB update

        final frame = YamuxFrame.windowUpdate(1, updateSize);
        await stream.handleFrame(frame);

        expect(
          stream.currentRemoteReceiveWindow,
    ...
    group('lifecycle', () {
      ...
      test('closes stream gracefully', () async {
        ...
        if (sentFrames.length == 2) {
          expect(
            sentFrames[0].type,
            equals(YamuxFrameType.dataFrame),
      ...
      test('focused closure test', () async {
        ...
        ); // Slightly longer delay for state finalization
        print(
    ...
    group('error handling', () {
      ...
      test('handles invalid frame types', () async {
        ...
        await expectLater(
          () async => stream.handleFrame(invalidFrame),
